...
apptainer run --nv \
  ...
  --bind ${WORK_DIR}/output:/output/set5 \
  --bind ${SCHEDULES_DIR}:/app/RFdiffusion/schedules \
  ${SIF_PATH} \
  inference.output_prefix=/output/set5/TfR1_set5_binder \
  inference.model_directory_path=/app/RFdiffusion/models \
  inference.input_pdb=/input/TfR1.pdb \
  'contigmap.contigs=[A121-759/0 70-120]' \
  'ppi.hotspot_res=[A208,A210,A211,A212,A215]' \
