...
**Date:** 2026-06-09
**Priority:** LOW (no rush — fits around current work)

## Why

We're applying to the NVIDIA Inception program (free GPU credits + BioNeMo access for the Bindlix campaign). The pitch deck needs a defensible total GPU-hours figure across all 3 clusters. Frontenac is verified at **1,891 GPU-hours / 921 jobs**. We need Nibi's number to complete the 3-cluster total — you ran the bulk of the TfR1 campaign (2,051 trajectories), so your share is substantial.

## Action

Run this one-liner on Nibi and report the output back via inbox to Frontenac:
...
- If Nibi's `AllocTRES` uses a different gres key (e.g. `gres/gpu:h100=N`), adjust the regex to `gres\/gpu[:=][a-z0-9]*=?[0-9]+` or just report raw `sacct` output and I'll parse it.
- Also note the GPU type(s) used (H100? A100?) if easy.
